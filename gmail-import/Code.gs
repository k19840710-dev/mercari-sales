...
function extractEmailInfo_(apiKey, subject, body, existingCards, messageDate) {
  ...
  const prompt = [
    ...
    '',
    '注意: 返品・返金に関するメールには性質が異なる2種類があるので区別してください。',
    '',
    '(a) カード会社・決済アカウント発行者（例:「三井住友カード」「メルカード」など、実際に',
    '　 カードや決済アカウントとして家計簿に登録されている主体）からの、返金がそのカード・',
    '　 アカウントへ確定的に入金されたことの通知。多くの場合、通常の利用確定通知と全く同じ',
    '　 件名・テンプレート（例:「ご利用のお知らせ」）で届き、店舗名の欄に「（返品）」等の',
    '　 記載がある、または「返金がありました」のように完了・確定を示す言い方をしています。',
    '　 → これは is_purchase_notification を true のまま、is_refund を true にしてください',
    '　 （このメール自体をスキップしないこと。amountには符号なしの金額をそのまま入れてください）。',
    '',
    '(b) 店舗やPayPal等の決済代行会社自身から届く、返金の「処理状況」だけを知らせるメール',
    '　 （例:「返金を処理中」「○月○日までに返金されます」のように、これから返金される',
    '　 予定・処理中であることを伝えるだけで、カードや決済アカウントへの入金がまだ確定して',
    '　 いないもの）。これは家計簿への登録対象にはなりません。実際の入金は後日、上の(a)の',
    '　 ようなカード会社側の確定通知として別途届くので、そちらで捕捉します。',
    '　 → これは is_purchase_notification を false にしてください（登録も確認待ちも不要です）。',
    '',
...
// カード会社からの返金確定通知（例:「メルカードご利用分の返金がありました」）も
// この安全網を通す必要があるので、返金確定を示す言い回しも含めている。
const USAGE_PHRASE_PATTERN = /(ご利用がありました|ご利用のお知らせ|利用速報|ご利用いただき|ご利用金額|カードのご利用|(の|で)(お)?支払いが?ありました|決済されました|決済が完了|お支払い(の)?確認|ショッピング利用|返金がありました|ご利用分の返金|返金いたしました|返金が完了)/;
